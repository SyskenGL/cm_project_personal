...
from cm_bridge.low_level import pack_sound_sensors_request
...
class CMSoundSensors(CMNode):
    ...
    def __init__(self, name='cm_sound_sensors'):
        ...
        self.__sound_sensors_server = actionlib.SimpleActionServer(
            '~sound_sensors_server',
    ...
    def __is_request_ignorable(self, request):
        if self.__sound_sensors_info is None:
            return False
        if request.enabled != self.__sound_sensors_info.enabled:
            return False
        if request.auto_follow != self.__sound_sensors_info.auto_follow:
            return False
        return True
    ...
    def __wait_for_serial_response(self, assigned_id):
        while not rospy.is_shutdown():
            ...
            if assigned_id == response.id:
                self.__update_sound_sensors_info(
                    response.robot_info.sound_sensors_info
                )
